...
public class MercurialEye extends ItemMode implements IExtraFunction
{
	public MercurialEye()
	{
		super("mercurial_eye", (byte)4, new String[] {"Extension", "Normal", "Transmutation"});
		this.setNoRepair();
	}
	
	final private int EXTENSION_MODE = 0;
	final private int NORMAL_MODE = 1;
	final private int TRANSMUTATION_MODE = 2;
				...
				for (int x = (int) box.minX; x <= (int) box.maxX; x++)
				{
					for (int y = (int) box.minY; y <= (int) box.maxY; y++)
					{
						for (int z = (int) box.minZ; z <= (int) box.maxZ; z++)
						{
							...
							if (mode == NORMAL_MODE && (oldBlock == Blocks.air || oldBlock.isReplaceable(world, x, y, z)))
							{
								if (kleinEmc < reqEmc)
									break;
							...
							}else if (mode == EXTENSION_MODE && (oldBlock == Blocks.air || oldBlock.isReplaceable(world, x, y, z)))
							{
								if (kleinEmc < reqEmc)
									break;
								if (PlayerHelper.checkedPlaceBlock(((EntityPlayerMP) player), x, y, z, newBlock, newMeta))
	...
	public void addKleinEMC(ItemStack eye, int amount)
	{
	...
	public void removeKleinEMC(ItemStack eye, int amount)
	{
		NBTTagList list = eye.stackTagCompound.getTagList("Items", NBT.TAG_COMPOUND);
		...
		for (int i = 0; i < list.tagCount(); i++)
		{
			...
			{
				NBTTagCompound tag = nbt.getCompoundTag("tag");
				if (tag.getDouble("StoredEMC") - amount >= 0)
				{
	...
	public ItemStack[] getInventory(ItemStack eye)
	{
