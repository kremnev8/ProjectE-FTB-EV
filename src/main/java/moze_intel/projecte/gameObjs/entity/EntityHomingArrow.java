...
public class EntityHomingArrow extends EntityHomingArrowBase {
    ...
    public EntityHomingArrow(World world, EntityLivingBase shooter, float par3)
    {
        super(world, shooter, par3);
        if (shooter instanceof EntityPlayer)
        {
            canPickup = !((EntityPlayer) shooter).capabilities.isCreativeMode;
        }
        
    }
    
    public EntityHomingArrow(World world, EntityLivingBase shooter, float par3, int mode)
    {
        super(world, shooter, par3);
        this.mode = mode;
        if (shooter instanceof EntityPlayer)
    ...
    {
        onEntityUpdate();
        if (!canPickup) this.canBePickedUp = 0;
        ...
        if (ticksExisted > 3 && hasTarget() && !this.inGround)
        {
            ...
            // Find the angle between the direct vec and arrow vec, and then
            // clamp it so it arcs a bit
            double theta = wrap180Radian(arrowMotion.angle(lookVec));
            theta = clampAbs(theta, Math.PI / 2); // Dividing by higher numbers
                                                  // kills accuracy
    ...
    {
        List<EntityLivingBase> candidates = worldObj.getEntitiesWithinAABB(targets[mode], this.boundingBox.expand(8, 8, 8));
        ...
        {
            if (!candidates.get(0).getCommandSenderName().equals(shootingEntity.getCommandSenderName()))
            {
                dataWatcher.updateObject(DW_TARGET_ID, candidates.get(0).getEntityId());
            }else if (candidates.size() > 1 && candidates.get(1) != null)
            {
                dataWatcher.updateObject(DW_TARGET_ID, candidates.get(1).getEntityId());
            }
        }
    ...
    {
        return ((EntityLivingBase) worldObj.getEntityByID(dataWatcher.getWatchableObjectInt(DW_TARGET_ID)));
    }
